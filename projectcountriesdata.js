...
addTopCountriesPopulation = function () {
	table.innerHTML = ""
    textheading.innerHTML = '10 most populated countries in the world'
	let topCountriesPopulation = countries_data.sort((a, b) => b.population - a.population);
	const world_population = countries_data.reduce((sum, { population }) => sum + population, 0);
    addFirstRowTopCountries(world_population)
	addRowInTable(topCountriesPopulation , world_population)
}

function sortLanguage(){
     table.innerHTML = ""
	 textheading.innerHTML = '10 most spoken languages in the world'
	 let allLang = []
	 countries_data.forEach((value)=> {
		allLang = allLang.concat(value.languages)
	 })
	 const singleLang = new Set(allLang)
	 const total_lang = singleLang.size
	 const data =[]
	 singleLang.forEach((value)=>{
		let count = 0
		...
		data.push({name: value , population:count})
	 })
    let sortlang = data.sort((a, b) => b.population - a.population)
    addRowInTable(sortlang, total_lang)	
}

addFirstRowTopCountries = function (total_lang) {
	let firstRow = table.insertRow(0)
	let firstRowCell1 = firstRow.insertCell(0)
	let firstRowCell2 = firstRow.insertCell(1)
	let firstRowCell3 = firstRow.insertCell(2)
	firstRowCell1.innerHTML = `world`
	firstRowCell2.classList.add("bars")
	firstRowCell3.innerHTML = `${numFormatter.format(total_lang)}`
	firstRowCell1.classList.add("country-name")
	firstRowCell3.classList.add("population-count")
}

addRowInTable = function(topTen,world_population){
	for (i = 0; i < 10; i++) {
		let row = table.insertRow(-1)
		let cell1 = row.insertCell(0)
		let cell2 = row.insertCell(1)
		let cell3 = row.insertCell(2)
		cell1.classList.add("country-name")
		cell3.classList.add("population-count")
		let bar = document.createElement('div')
		bar.style.backgroundColor = 'orange';
		bar.style.height = 40
		bar.style.borderRadius = 5 + 'px'
		console.log(topTen[i])
		bar.style.width = topTen[i].population / world_population * 100 + '%'
		cell2.append(bar)
		cell1.innerHTML = topTen[i].name
		cell3.innerHTML = numFormatter.format(topTen[i].population)
	}
